...
puppeteer.use(recaptcha, StealthPlugin());

const cp = require('crypto');
const toughCookie = require('tough-cookie');
const { USKeyboardLayout } = import('puppeteer');
...
function getURLCacheKey() {
  return `puppeteer-bot-2a-url-parsed-cache:${parseInt(
...
async function puppeteerErrorRetry(fn) {
  ...
  for (let i = 0; i < 300; i += 1) {
    try {
      ...
      if (
        !/Cannot find context with specified id/.test(error.message) &&
        !/Session closed\. Most likely the page has been closed/.test(
          error.message,
        ) &&
        !/Protocol error \(Runtime\.callFunctionOn\): Target closed/.test(
          error.message,
        ) &&
        !/Session error \(Runtime\.callFunctionOn\): Message timed out/.test(
          error.message,
        ) &&
...
async function fetchURLResponse({ req, proxy, raw = false, bot = {} }) {
  ...
  headers = Object.keys(headers)
    .filter(
      k =>
        !~OmittedHeaders.concat(
          bot.anonymizeReferer ? ['Referer'] : [],
        ).indexOf(k),
    )
    .sort((a, b) => {
      ...
      return (
        (~orderA ? orderA : Number.MAX_SAFE_INTEGER) -
  ...
  const jar =
    bot && bot.getRequestCookieJar && (await bot.getRequestCookieJar());
...
function getBrowserFingerprint(buid) {
  ...
  const buidHash = cp.createHash('sha512').update(buid).digest();
...
class PuppeteerBot2a {
  constructor({
    ...
    this.monitorDetects = monitorDetects;

    this.logger = createLogger(this.label);
  }
  ...
  static async disguisePage(
    ...
    await page.evaluateOnNewDocument(async (fingerprint, LO, D) => {
    ...
    await browserContext.overridePermissions(page.url(), [
      'geolocation',
      'midi',
      'notifications',
      'microphone',
    ...
    if (_.isFinite(geolocation.latitude) && _.isFinite(geolocation.longitude)) {
      debug(
        `setting-geolocation:${geolocation.latitude}${geolocation.longitude}`,
      );
  ...
  async waitForNavigation({
    ...
    if (!Array.isArray(contentLevel) || contentLevel.length !== 2)
      throw new Error('unknown-content-level');
    const timeoutError = new Error('waitForNetworkIdle timeout');

    function checkLifecycle(
      frame,
      expectedLifecycle,
      expectedLifecycleForMain = [],
    ) {
      const lifecycles = expectedLifecycle.concat(expectedLifecycleForMain);
      // eslint-disable-next-line no-underscore-dangle
      debug(
        'frame lifecycle:',
        Array.from(frame._lifecycleEvents).join(','),
        'target:',
        lifecycles,
      );
    ...
    for (const started = Date.now(); Date.now() - started < timeout; ) {
      this.requestIds = this.requestIds.filter(
        r => Date.now() - r.at < timeout,
      );

      debug('checkLifecycle start');
      const metLifecycle = checkLifecycle(
        this.page.mainFrame(),
        contentLevel[0],
        contentLevel[1],
      );
      ...
      if (
        !doCheckLifecycle &&
        this.page.mainFrame()._loaderId !== initialLoaderId
      ) {
  ...
  async debugCapture(key = uuid.v4()) {
    ...
    return this.page.screenshot({
      path: `/tmp/ss_${key}_${Date.now()}.png`,
      fullPage: true,
    });
  }

  async goto(u, o) {
    await this.page.goto(u, o);
  ...
  async checkElementHandle(handle, value = true) {
    ...
    // eslint-disable-next-line no-await-in-loop
    while (
      (await puppeteerErrorRetry(async () =>
        (await handle.getProperty('checked')).jsonValue(),
      )) !== value
    ) {
      // eslint-disable-next-line no-await-in-loop
  ...
  async $$safeEval(q, fn, ...args) {
    try {
      return await puppeteerErrorRetry(async () =>
        this.page.$$eval(q, fn, ...args),
      );
    } catch (error) {
  ...
  async $$(selector, shouldIncludeInvisible = false) {
    ...
    setImmediate(() => {
      ...
      // map batches
      const argKeyMap = batches.reduce(
        (m, c) =>
          Object.assign(m, {
            [c.key]: [...(m[c.key] || []), c],
          }),
          ...
          const aggregatedMap = await puppeteerErrorRetry(async () => {
            const map = await (
              await this.page.evaluateHandle(
                ...
                Object.values(argKeyMap).map(a =>
                  _.pick(a[0], 'selector', 'includeInvisible', 'key'),
                ),
              )
            ).getProperties();

            await Promise.all(
              Object.keys(argKeyMap).map(async key => {
                let result = map.get(key);
                if (result) {
                  // make it array of elements
                  result = [...(await result.getProperties()).values()].map(e =>
                    e.asElement(),
                  );
                }
                map.set(key, result);
    ...
    return puppeteerErrorRetry(async () => {
      ...
      if (
        !(
          ...
          (await this.page.evaluate(isVisible, handle))
        )
      )
        return false;
  ...
  async fillElementHandle(handle, opt) {
    return puppeteerErrorRetry(async () => {
      ...
      const maxLength = await this.page.evaluate(e => {
        e.value = '';
        const len = parseInt(
          e.maxLength || e.getAttribute('maxLength') || 1000,
          10,
        );
  ...
  async $$click(selector) {
    ...
    for (let i = 0; i < elementHandles.length; i += 1) {
      // eslint-disable-next-line no-await-in-loop
      result =
        (await this.clickElementHandle(elementHandles[i], {
          selector,
        })) || result;
    }
    return result;
  ...
  async selectByValue(opt) {
    const elementHandles = await this.page.$x(
      `//select[contains(text(), '${opt.replace(/'/g, "', \"'\", '")}')]`,
    );
    if (!elementHandles) return false;
    return this.clickElementHandle(elementHandles[0]);
  }

  async clickByLinkText(url) {
    const elementHandles = await this.page.$(`a[href*="${url}"]`);
    if (!elementHandles) return false;
    return this.clickElementHandle(elementHandles);
  }
  ...
  async waitForNavigationOrSelector(
    selector,
    waitForNavOption = {
      ...
      await Promise.race([
        ...
        (async () => {
          while (!resolved) {
            // eslint-disable-next-line no-await-in-loop
            if (
              await PuppeteerBot2a.puppeteerErrorRetry(async () =>
                this.page.$(selector),
              )
            )
  ...
  async clickElementHandle(handle) {
    return puppeteerErrorRetry(async () => {
      if (!handle) {
        this.logger.warn('clickElementHandle-empty-handle');
        return false;
      }

      const boundingBox = await handle.boundingBox();
      if (!boundingBox) {
        this.logger.warn('clickElementHandle-empty-bounding-box');
        return false;
      }
      ...
      for (let i = 0; ; i += 1) {
        ...
        targetX = parseInt(
          x + Math.min(Math.random() * width, width - 2) + 1,
          10,
        );
        targetY = parseInt(
          y + Math.min(Math.random() * height, height - 2) + 1,
          10,
        );
  ...
  async waitForSelector(opt, options) {
    return this.page.waitForSelector(opt, options);
  }
  ...
  async healthCheckRepeater() {
    try {
      ...
      this.logger.error('health-check-repeater-error');
    } finally {
  ...
  async healthCheck() {
    ...
    if (Date.now() - this.healthCheckStartedAt > this.browserTimeout) {
      this.logger.error('health-check-failed-browser-timeout');
      return false;
    }
    ...
    try {
      if (this.interaction && this.interaction.isUp) {
        if (!(await this.interaction.checkValid())) {
          this.logger.error('health-check-failed-interaction-invalid');
          return false;
        }
      }
    } catch (error) {
  ...
  async startBrowser() {
    ...
    if (this.proxy) {
      ...
      try {
        ...
        this.logger.verbose(
          `fingerprint-maxmind-geoip-latitude-${geolocation.latitude}`,
        );
        this.logger.verbose(
          `fingerprint-maxmind-geoip-longitude-${geolocation.longitude}`,
        );
        ...
        this.logger.error(
          `maxmind-lookup-error-${Buffer.from(error.message).toString(
            'base64',
          )}`,
          {
            error,
    ...
    if (!this.browser) {
      const DISPLAY = this.preferNonHeadless
      ...
      if (this.loadExtensionName && this.preferNonHeadless) {
        const qualifiedPathToExtension = path.join(
          __dirname,
          `./extensions/${this.loadExtensionName}`,
        );
    ...
    if (!this.page) {
      ...
      this.page.on('requestfailed', async interceptedReq => {
        this.requestIds = this.requestIds.filter(
          r => r.id !== interceptedReq.id,
        );
        const rurl = interceptedReq.url();
        if (this.incompleteRequests) {
          // eslint-disable-next-line no-underscore-dangle
          this.incompleteRequests = this.incompleteRequests.filter(
            r => r !== interceptedReq,
          );
        }
        // eslint-disable-next-line no-underscore-dangle
        if (
          /net::ERR_ABORTED/.test(interceptedReq._failureText) ||
          /net::ERR_CONNECTION_CLOSED/.test(interceptedReq._failureText) ||
          /net::ERR_CONNECTION_REFUSED/.test(interceptedReq._failureText)
        )
          return;
      ...
      this.page.on('request', async interceptedReq => {
        ...
        if (this.interceptRequest && this.interceptRequest(interceptedReq))
          return null;

        const rurl = this.requestURLReplacer(interceptedReq);
        if (!rurl) return interceptedReq.abort();

        if (this.requestSoftAbortRules.find(r => r.test(rurl)) !== undefined)
        ...
        if (
          this.requestHardAbortRules.find(regex =>
            interceptedReq.url().match(regex),
          )
        ) {
        ...
        try {
          ...
          if (
            (!this.urlCacheSkipRules ||
              !this.urlCacheSkipRules.find(r => r.test(rurl))) &&
            /^http[s]?:\/\/(.*)\/.+\.(js|css|woff|woff2|png|jpg|gif|bmp|ico)(\?.*)?$/i.test(
              rurl,
            )
          ) {
            return interceptedReq.respond(
  ...
  async stopBrowser() {
    try {
      ...
      this.logger.error('failed-page-close');
    } finally {
      this.page = null;
    }
    ...
    try {
      ...
      this.logger.error('failed-browser-close');
    } finally {
      this.browser = null;
    }
  ...
  async getRequestCookieJar() {
    ...
    jar._jar = toughCookie.CookieJar.fromJSON({
      looseMode: true,
      // eslint-disable-next-line no-underscore-dangle
      cookies: (await this.client.send('Network.getAllCookies')).cookies.map(
        c => ({
          key: c.name,
          value: c.value,
  ...
  async loadCookies() {
    if (!this.parsedCredential || !this.parsedCredential.cookies || !this.page)
      return;
      ...
      await Promise.all(
        this.parsedCredential.cookies.map(async c => {
          ...
          try {
            ...
            this.logger.error('failed-setting-cookie');
          }
        }),
      );
    }
  ...
  async loadUserData() {
    if (
      !this.parsedCredential ||
      !this.parsedCredential.chromeUserData ||
      !this.userDataDir ||
      this.parsedCredential.chromeUserData.length < 1
    )
      return;
    ...
    stream.end(
      Buffer.isBuffer(this.parsedCredential.chromeUserData)
        ? this.parsedCredential.chromeUserData
        : Buffer.from(this.parsedCredential.chromeUserData, 'base64'),
    );
  }
...
PuppeteerBot2a.ContentLevel = {
  CONTENTFUL: [[], ['firstContentfulPaint']],
  SKELETON: [[], ['DOMContentLoaded']],
  MEANINGFUL: [[], ['firstMeaningfulPaint']],
  FULL: [['load', 'networkidle'], []],
};
